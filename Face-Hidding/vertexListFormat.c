...
principalVLF (char * name)
{
  ...
  principal3d(vertexList,edgeList,faceList,nVertexList,nEdgeList,nFaceList,name);
}
...
getVertexList (int raws)
{
  ...
  struct vertex tmp;
  FILE *fp = fopen ("model.raw", "r");
  vertexList = (struct vertex *) malloc ((sizeof (struct vertex)) * (raws * 3));
  ...
  for (i = 0, k=0; i < raws * 3; i++)
    {
      ...
      for(j=0;j<i;j++)
      {
      	...
      	{
      		if((tmp.x==vertexList[j].x)&&(tmp.y==vertexList[j].y)&&(tmp.z==vertexList[j].z))
      		{
     	 		exist=1;
      			break;
      		}
      ...
      if(exist==0)
      {
      	...
      	vertexList[k].zBuff=vertexList[k].z=tmp.z;
      	vertexList[k].w=1.0;
...
inicializateVertexList(struct vertex *vertexList, int raws)
{
	for(int i=0;i<raws*3;i++)
		vertexList[i].x=vertexList[i].y=vertexList[i].z=vertexList[i].w=vertexList[i].hash=0.0;
}
...
printVertexList(struct vertex *vertexList)
{
  ...
  for(int i=0;i<nVertexList;i++)
	{
		printf("%d			%lf			%lf			%lf\n",vertexList[i].num,vertexList[i].x,vertexList[i].y,vertexList[i].z);
...
{
  struct vertex *new=(struct vertex *)malloc(sizeof(struct vertex)*nVertexList);
  for(int i=0;i<nVertexList;i++)
    new[i]=vertexList[i];
  free(vertexList);
  return new;
...
getEdgeList (int raws, struct vertex *vertexList)
{
  ...
  {
    fscanf (fp, "%lf %lf %lf %lf %lf %lf %lf %lf %lf ", &v1.x, &v1.y, &v1.z, &v2.x, &v2.y, &v2.z, &v3.x, &v3.y, &v3.z);
    v1.w = 1.0;
    ...
    for(j=0;j<nVertexList;j++)
    {
      ...
      {
        if((v1.x==vertexList[j].x)&&(v1.y==vertexList[j].y)&&(v1.z==vertexList[j].z))
        {
          a1=&vertexList[j];
          break;
        }
    ...
    for(j=0;j<nVertexList;j++)
    {
      ...
      {
        if((v2.x==vertexList[j].x)&&(v2.y==vertexList[j].y)&&(v2.z==vertexList[j].z))
        {
          a2=&vertexList[j];
          break;
        }
    ...
    for(j=0;j<nVertexList;j++)
    {
      ...
      {
        if((v3.x==vertexList[j].x)&&(v3.y==vertexList[j].y)&&(v3.z==vertexList[j].z))
        {
          a3=&vertexList[j];
          break;
        }
...
{
  struct edge *new=(struct edge *)malloc(sizeof(struct edge)*nEdgeList);
...
printEdgeList(struct edge *edgeList)
{
  ...
  for(int i=0;i<nEdgeList;i++)
  {
    printf("%d      %d     %d\n",edgeList[i].num,edgeList[i].start->num,edgeList[i].end->num);
    fprintf(fp,"%d      %d     %d\n",edgeList[i].num,edgeList[i].start->num,edgeList[i].end->num);
  }
  fclose(fp);
}
...
getFaceList (int raws, struct vertex *vertexList, struct edge *edgeList)
{
  ...
  for (i = 0, k=0; i < raws*3; i+=3, k++)
  {
    ...
    for(int m=0;m<nEdgeList;m++)
    {
      ...
      {
        if((e1.start==edgeList[m].start)&&(e1.end==edgeList[m].end))
        {
          b1=&edgeList[m];
          break;
        }
    ...
    for(int m=0;m<nEdgeList;m++)
    {
      ...
      {
        if((e2.start==edgeList[m].start)&&(e2.end==edgeList[m].end))
        {
          b2=&edgeList[m];
          break;
        }
    ...
    for(int m=0;m<nEdgeList;m++)
    {
      ...
      {
        if((e3.start==edgeList[m].start)&&(e3.end==edgeList[m].end))
        {
          b3=&edgeList[m];
          break;
        }
...
printFaceList(struct face *faceList)
{
  ...
  {
    printf("%d      %d     %d     %d\n",faceList[i].num,faceList[i].e1->num,faceList[i].e2->num,faceList[i].e3->num);
    fprintf(fp,"%d      %d     %d     %d\n",faceList[i].num,faceList[i].e1->num,faceList[i].e2->num,faceList[i].e3->num);
  }
  fclose(fp);
}