...
double
obtainSlope (int **points)
{
  return (points[1][1] - points[0][1]) / (points[1][0] - points[0][0]);
}

double
obtainB (int **points, double slope)
{
  return points[0][1] - (points[0][0] * slope);
}

void
calculate_line(int **points,int slope, int b, int **raster)
{
  int x=0;
  int y=0;
  x=points[0][0];
