...
void principalVLF(char *name, char *alfa, char *beta, char *gamma, char *vScale,
		  char *translationX, char *translationY, char *r, char *g,
		  char *b)
{
	...
	for (i = 0, k = 0; i < raws * 3; i++) {
		...
		for (j = 0; j < i; j++) {
			if (tmp.hash == vertexList[j].hash) {
				if ((tmp.x == vertexList[j].x)
				    && (tmp.y == vertexList[j].y)
...
void inicializateVertexList(struct vertex *vertexList, int raws)
{
	for (int i = 0; i < raws * 3; i++)
		vertexList[i].x = vertexList[i].y = vertexList[i].z =
		    vertexList[i].w = vertexList[i].hash = 0.0;
}
...
void printVertexList(struct vertex *vertexList)
{
	...
	for (int i = 0; i < nVertexList; i++) {
		fprintf(fp,
			...
			vertexList
			[i].
			num, vertexList[i].x, vertexList[i].y, vertexList[i].z);
	}
	fclose(fp);
}
...
void printFaceList(struct face *faceList)
{
	...
	for (int i = 0; i < nFaceList; i++) {
		fprintf(fp, "%d      %d     %d     %d\n", faceList[i].num,
			faceList[i].e1->num, faceList[i].e2->num,
			faceList[i].e3->num);
	}
	fclose(fp);
}
